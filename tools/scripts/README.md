...
| `clamp_dem.py` | OSGeo4W Shell: `python clamp_dem.py "<dem>.tif" [--in-place]` | Turn impossible elevations in a DEM/DTM into nodata. Bolzano's DOM has a bad spot at ~694965 E / 5168505 N that has surfaced in two AOIs (Seceda read 1,375,339 m, Sassolungo 467,766 m) - it makes QGIS render the raster solid black, and Blender's Displace, which ignores the nodata tag, would build a several-hundred-kilometre spike. `--dry-run` reports and locates them without writing. |
| `veneto_lidar_vrt.py` | OSGeo4W Shell: `python veneto_lidar_vrt.py "<tile folder>" --out fine.tif --te XMIN YMIN XMAX YMAX` | Mosaic Regione Veneto's LiDAR `.asc` tiles (from the IDT downloader, which ships **no `.prj`**) into a VRT, then reproject and clip to an AOI. Detects the source CRS from the eastings — the downloader's "Fuso 12" is `EPSG:7795`, whose 3,000,000 false easting matches no UTM zone. Normalises `xllcenter` to a corner, reports what percentage of the AOI the tiles actually cover, and clamps absurd elevations to nodata. `--dry-run` reports without writing. |
...
All six need PyQGIS + the GDAL Python bindings that QGIS ships, nothing else.
...
They are documented in that project's `HOWTO.md`, which is the master procedure
